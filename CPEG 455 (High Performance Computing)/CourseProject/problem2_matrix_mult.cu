...
//change to create bank conflicts
#define offset 0
...
__global__ void gpu_square_matrix_mult(int *d_a, int *d_b, int *d_result, int n) {
    ...
    //shared memory to avoid conflicts
    __shared__ int tile_a[BLOCK_SIZE][BLOCK_SIZE];

    __shared__ int tile_b[BLOCK_SIZE][BLOCK_SIZE];

    //offset misaligns memory and causes bank conflict
...
int main(int argc, char const *argv[]){
    ...
    float gpu_square_elapsed_time_ms;
    ...
    printf("Average time elapsed on tiled matrix multiplication of %dx%d . %dx%d on GPU: %f ms.", m, n, n, k, gpu_square_elapsed_time_ms/100);



    // validate results by comparing to non-tiled matrix calculation
    int all_ok = 1;
    for (int i = 0; i < m; ++i)
    {
        for (int j = 0; j < k; ++j)
